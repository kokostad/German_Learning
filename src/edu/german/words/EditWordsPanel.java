...
public class EditWordsPanel extends JPanel {
	...
	public EditWordsPanel(String labelInfo1, String labelInfo2, String[] selectionList) {
		...
		word = new OneEditField.Builder().withTitle(Titles.setTitel("KEY_WORD")).withHint(Titles.setTitel("KEY_WORD"))
				.withFontSize(ss.DEFAULT_FONT_SIZE).withWidth(ss.WORD_FIELD_WIDTH).withHeight(ss.EDIT_FIELD_HEIGHT)
				.build();
		this.add(word);

		if (labelInfo2 != null) {
			meaning = new OneEditField.Builder().withTitle(Titles.setTitel("MEANING"))
					.withHint(Titles.setTitel("WRITE_MEANING")).withFontSize(ss.DEFAULT_FONT_SIZE)
					.withWidth(ss.WORD_FIELD_WIDTH).withHeight(ss.EDIT_FIELD_HEIGHT).build();
			this.add(meaning);
		}
